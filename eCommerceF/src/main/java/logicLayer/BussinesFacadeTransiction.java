...
public class BussinesFacadeTransiction {
	...
	public SessionAcess createAcess(String sessionTk, Long createdDate) {

		SessionAcess sessionAcess = new SessionAcess();
		int siteId = site.acessSite();
		int clientId = client.createClient(siteId);
		session.createSession(siteId, sessionTk, clientId, createdDate);
	...
	public void addProduct(SessionAcess sessionAcess, int productId, String cartTk) {
		...
		/*	Stm_site_cart carts = cart.getCartByName(cartTk, sessionAcess.getSessionId());
			cart.addCart(carts.getCart_tk(), productId, sessionAcess.getSiteId());*/
	...
	/*public void getOffProduct(SessionAcess sessionAcess, int productId, String cartTk) {
		
		
		
		Stm_product products = product.getProducts(productId);
		Stm_site_cart carts = cart.getCartByName(cartTk,sessionAcess.getSessionId());
		
		cart.removeCartProduct(carts.getCart_tk(),products.getProduct_id(),sessionAcess.getSiteId(),)
